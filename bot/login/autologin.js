...
exports.handleRelogin = async function() {
    try {
        ...
        if (
            ...
            logger('Missing account information in config.json', '[ ERROR ]');
            return false;
        }
        const { email, password, "2FASecret": twoFA } = config.facebookAccount;
        logger('Try logging in again...', '[ LOGIN ]');     
        ...
        if (!loginResult.status) {
            logger(`Login failed: ${loginResult.message}`, '[ ERROR ]');
            return false;
        }
...
logger('New cookies have been updated.', '[ COOKIE ]');
global.account = { cookie: newCookie };

const tokensDir = join(process.cwd(),'./bot/data');
...
if (fs.existsSync(tokensPath)) {
    tokens = JSON.parse(fs.readFileSync(tokensPath, 'utf8'));
        ...
        if (loginResult.data.access_token_eaad6v7) {
            logger('EAAD6V7 token update successful.', '[ TOKEN ]');
            tokens.EAAD6V7 = loginResult.data.access_token_eaad6v7;
            if (!global.account) global.account = {};
            global.account.cookie = newCookie;
        }
        if (loginResult.data.access_token) {
            logger('EAAAAU token update successful', '[ TOKEN ]');
            tokens.EAAAAU = loginResult.data.access_token;
            // global.account.token.EAAAAU = loginResult.data.access_token;          
        }
        fs.writeFileSync(tokensPath, JSON.stringify(tokens, null, 2));       
        // Success: By default, the program does NOT exit. It only exits if explicitly configured.
        const shouldRestart = process.env.AUTOLOGIN_RESTART === 'true'
            || (global.GoatBot && global.GoatBot.config && global.GoatBot.config.autoLogin && global.GoatBot.config.autoLogin.restartAfterSuccess === true);

        if (shouldRestart) {
            logger('Login successful, restarting...', '[ SUCCESS ]');
            if (global.client) {
        ...
        logger(`Error when logging in again: ${error.message}`, '[ ERROR ]');
        return false;
    }
};
...
exports.handleError = async function(error) {
    ...
    if (errorStr.includes("601051028565049") || 
        ...
        logger("Facebook alert detected, in progress bypassing...", "[ WARNING ]");
       if (!global.account || !global.account.cookie) {
    logger("Cookie not found in global.account", "[ WARNING ]");
    return false;
}
        ...
        try {
            ...
            if (response.status === 200 || response.status === 302) {
                ...
                logger("Successfully cleared the alert!!!", "[ SUCCESS ]");
                return true;
            }
            return false;
        } catch (bypassError) {
            logger("Error when passing the checkpoint: " + bypassError.message, "[ ERROR ]");
            return false;
        }
    }
    return false;
