...
if (body.startsWith(prefix)) {
...
else if (isAdminPrefixEnable && body.startsWith(adminPrefix)) {
	currentPrefix = adminPrefix;
						...
						// Check if thread is spam banned
	...
	if (!hideNotiMessage.commandNotFound) {
		...
		return true;
												...
												try {
															 ...
															 (async () => {
				if (!globalData || typeof globalData.get !== "function") return;
...
if (threadData?.data?.antilink === true && body && linkRegex.test(body)) {
 if (role < 1) {
	...
	if (!global.temp.linkWarn)
	 global.temp.linkWarn = {};

	if (!global.temp.linkWarn[threadID])
	 global.temp.linkWarn[threadID] = {};

	const warn = global.temp.linkWarn[threadID][senderID] || 0;
	global.temp.linkWarn[threadID][senderID] = warn + 1;
 ...
 message.reply("⚠️ Cannot remove user. Bot may not be admin.");
	 ...
	 await message.reply(`⚠️ Links are not allowed\nWarning ${global.temp.linkWarn[threadID][senderID]}/${maxWarn}`);
	}
...
if (antiRaid.enable && event.logMessageType === "log:subscribe") {
 ...
 if (addedUsers.length > (antiRaid.maxAdd || 5)) {
	...
	const { userID, statuses, timestamp } = event;
	if (!userID) return;

	if (!global.temp.presence)
		global.temp.presence = {};

	global.temp.presence[userID] = {
		status: statuses === 2 ? "online" : (statuses === 0 ? "offline" : "idle"),
	...
	const { reader, threadID, time } = event;
	if (!reader || !threadID) return;

	if (!global.temp.readReceipts)
		global.temp.readReceipts = {};

	if (!global.temp.readReceipts[threadID])
		global.temp.readReceipts[threadID] = {};

	global.temp.readReceipts[threadID][reader] = time || Date.now();
}
								...
								*/
								async function typ() {
	const { from, isTyping, threadID } = event;
	if (!from || !threadID) return;

	if (!global.temp.typingUsers)
		global.temp.typingUsers = {};

	if (!global.temp.typingUsers[threadID])
		global.temp.typingUsers[threadID] = [];

	if (isTyping) {
		if (!global.temp.typingUsers[threadID].includes(from))
			global.temp.typingUsers[threadID].push(from);
	}
	else {
		global.temp.typingUsers[threadID] =
