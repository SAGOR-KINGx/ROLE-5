...
module.exports = async function (databaseType, threadModel, api, fakeGraphql) {
	...
	let dataCreated = await threadModel.findOne({ threadID: threadData.threadID }).lean();
	dataCreated = _.omit(dataCreated, ["_id", "__v"]);

	const existIndex = global.db.allThreadData.findIndex(t => t.threadID == dataCreated.threadID);
	if (existIndex == -1)
		global.db.allThreadData.push(dataCreated);
	else
		global.db.allThreadData[existIndex] = dataCreated;
